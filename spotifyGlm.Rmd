...
>
Logistics:
1. The R code is fully reproducible and attached as spotifyGLM.rmd (I've just include=FALSE) for the code lines. \
2. Some of the EDA was done in python, files attached in the folder EDA_python.
...
In this paper, I will model how a certain song become popular. I've chose the data-set [Spotify tracks dataset](https://www.kaggle.com/datasets/lehaknarnauli/spotify-datasets?ref=datascience.fm&select=tracks.csv), which consist hundred of thousands of songs which can be describe by mostly audio features. I will use generalized linear models framework for this task.
...
Our target variable is the popularity of each song. $Y$ is continuous variable (from 0 to 100). For modeling the logistic model, i will introduce another target variable ($is\_{popular}$), which have a binary outcome.
...
```{r all_feat_hist_show, echo=FALSE, out.width = '100%', include=TRUE, fig.align = 'center'}
...
- We have some outliers in num_of_artists, duration_ms.
...
This is a of the KDE of some interesting continues features. $KDE(X|Y=1)$, $KDE(X|Y=0)$ (Gaussian kernel), when $X$ is a certain feature and $Y=is\_popular$.
...
```{r corr, include=FALSE, eval=TRUE}
png("spotifyGLM/plots/corr.png")
...
```{r corr_show, echo=FALSE, eval=FALSE,fig.cap="", out.width = '55%'}
...
1. General info. \
2. Different metrics I've used. \ 
3. Different formulas I've used. \
4. Different GLM's I've used. \
...
Note that in terms of GLM's, this form can be simplified as a function of the family and the link function (for example in poisson glm with log link function).
...
The main problem with this model is over-dispersion. The Poisson distribution assumes that the mean and variance of the response variable are equal. If the variance is larger than the mean, it indicates over-dispersion. mean of the popularity is very not equal to the variance (mean=`r round(mean(df$popularity),2)`, var=`r round(var(df$popularity),2)`), which could indicate on over-dispersion (but this is not the right measurement for over-dispersion).
...
I chose to use the negative binomial model. Actually, asymptotically, the NB model will aim to the poisson model as the $r$ parameter goes to infinity. A short explanation from [Tim Barry](https://timothy-barry.github.io/posts/2020-06-16-gamma-poisson-nb/): \
...
The Logistic GLM is used for binary outcomes, like success/failure. It employs the logit link function to model the log-odds of success as a linear combination of predictors:

\[
\log\left(\frac{p}{1-p}\right) = \beta_0 + \beta_1 x_1 + \ldots + \beta_p x_p
\]

Here, \(p\) is the probability of success, and \(\beta_0, \ldots, \beta_p\) are coefficients. To interpret coefficients, exponentiate them:

- \(e^{\beta_j}\) is the multiplicative effect on odds for a 1-unit increase in \(x_j\).
- \(\frac{e^{\beta_j}}{e^{-\beta_j}}\) is the factor change in odds per 1-unit increase.

Coefficients greater than 1 increase odds; less than 1 decrease them. For example, if \(e^{\beta_1} = 2.5\), odds increase by 2.5 times for each unit increase in \(x_1\).
The most common link functions used in logistic regression are the logit, probit, and loglog link functions.
...
I will compare different logit, probit, loglog by ROC, BIC, AIC and deviance.
...
$$
\text{logit}(P(\text{is popular})) = \beta_0 + \beta_1 \times \text{duration min} + \beta_2 \times \text{num of artists} + \ldots + \beta_k \times \text{energy:mode1} + \alpha_{\text{artist}}
$$

where:

- $\beta_0$ to $\beta_k$ are the fixed effect coefficients for the predictor variables.
- $\alpha_{artist}$ represents the random effect for each artist.
...
comparison_df_nb_models <- calculate_comparison_df(nb_models, "nb_model")
comparison_df_lm_models <- calculate_comparison_df(lm_models, "lm_model")
models_pois_compare = comparison_df_poisson_models[,c("model_names","df_res","deviance","scaled_deviance","aic","bic")]
kable(models_pois_compare)
```
...
stargazer(poisson_model_reduced, coeftest(poisson_model_reduced, vcov = sandwich::vcovHC), type="latex",header=FALSE, single.row = TRUE, 
          ...
          font.size = "tiny", column.labels = c("Poisson", "Poisson robust std"), float = FALSE )
```

The errors of this model seems heteroskedastic, so I've run the model with robust standard errors.
...
We can see that there are some important features that becomes unsignificant with the sandwich method (Pval< 0.001) like mode and explicit.
...
As we suspected, the poisson model have big over-dispersion ($\phi$=`r round(dispersion_param,2)`, Pval<0.001).
As i describe in the [methods](#methods) section, I chose to handle this by applying a Negative - binomial model.

### Negative-binomial
I've chose the same model as the poisson. I've already stated that the comparison between those its a bit problematic (with some metrics).
The DHARMa package is an easy to use package that can be use for model analysis for glm's. The package can transform the residuals of any GL(M)M into a standardized space. Once this is done, we can visually assess / test residual problems such as deviations from the distribution, residual dependency on a predictor, heteroskedasticity or autocorrelation in the normal way.
Fundamental results for ordinary linear models about projections and orthogonality of fitted values and residuals do not hold exactly for GLM's, so using this package can very helpful in terms of GLMS.
...
Poisson vs NB coefficients         |  Poisson vs NB coefficients(sandwich estimators)
:-------------------------:|:-------------------------:
...
First, seems that the estimates for both of the models are pretty much the same (with slight difference in some of the coefficients). Interestingly, when we are not using robust standard errors, the CI of the poisson coefficients are much smaller than the NB model. But when we are using robust standard error, the CI becomes much bigger.
...
You can also see that the residuals for the Poisson model are more spread out than the residuals for the Poisson model. This is because the Poisson model allows for over-dispersion, which means that the variance of the data is greater than the mean. The NB model does not allow for over-dispersion, so the residuals for the NB model are more tightly clustered around zero.
...
```{r}
lm_model_reduced = glm(data=df_only_popular, formula = formula_3_reduced)
final_metrics_models = calculate_comparison_df(list(nb_model_reduced, poisson_model_reduced, lm_model_reduced), "dfd")
final_metrics_models$model_names = c("nb","poisson","lm")
final_metrics_models = final_metrics_models[,c("model_names","df_res","deviance","scaled_deviance","aic","bic")]
kable(final_metrics_models)
```
Lastly, i wanted to compare our models to the regular linear model (GLM with Gaussian family and identity link). 
Turns out that the linear model works out the best in almost all metrics. This could raise up a question, do we even need to use different generalized families for this problems?
I think that this comparison with 'raw' statistics such as deviance and AIC could be problematic in some cases. If our problem can be best solved with a certain link function and distribution family, we maybe need to leave our model comparison 'inside the family'.
Let's move on modeling this problem as a classification problem:
...
For fitting a logistic Generlized-linear-mixed-model, ive filtered the dataset for artists that have more than three songs in those years for making our resutls more stabilise. 
...
anova(glmm_logistic, logistic_model)
```
...
```{r, include=TRUE,results='asis'}
stargazer(glmm_logistic, logistic_model,type = "latex",header=FALSE, single.row = TRUE, 

          no.space = TRUE, # to remove the spaces after each line of coefficients

          column.sep.width = "3pt", # to reduce column width

          font.size = "tiny",float = FALSE)
```


The LRT test shows that the glmm model have significant better fit ($\chi^2_{stat} = 2841.3$)
The coefficients estimates and significance has changed dramatically with comparison to the new glmm model.
There are some coefficients with opposites estimates (GLM positive estimate, GLMM negative estimate), when looking into those coefficients, it seems like there are probably a lot of noise, because the significance is not high, and the estimate is pretty small.

Overall, the GLMM model is a more complex model than the logistic model, and it is able to account for more factors that may affect a song's popularity. However, the logistic model is simpler and easier to interpret, and it may be more appropriate for some datasets.
Based on the results of the two models, I would say that the best model is the GLMM model. This is because the GLMM model is able to account for more factors that may affect a song's popularity, and it is more likely to provide accurate results. 

Another interesting metric that i thought of is to compare the ranking of the absolute value of the coefficients.
The intercept of the models is different, yielding different interaction between the coefficients, I've tried to reduce this complexity by plotting the ranking of the coefficients of the model.
...
library(gridExtra)
library(ggpubr)
# png("spotifyGLM/plots/ranking_glmm.png")


# Create squared plots
g1 <- compare_coefficient_rankings(logistic_model, glmm_logistic, significance_by = "a", title = "Significance by GLM")
g2 <- compare_coefficient_rankings(logistic_model, glmm_logistic, significance_by = "b", title = "Significance by GLMM")

# Arrange the squared plots
# grid.arrange(g1, g2, ncol = 2, top = "Coefficients rank (absolute) between GLM and GLMM",
#              widths = c(4, 4)) 
print(ggarrange(g1, g2, ncol = 2,nrow=1, top = "Coefficients rank (absolute) between GLM and GLMM",
             common.legend=TRUE) )
# dev.off()
# ggsave("spotifyGLM/plots/ranking_glmm.png", plot=p)
```

```{r ranking, echo=FALSE, out.width = '80%', include=TRUE}
knitr::include_graphics("spotifyGLM/plots/ranking_glmm.png")
```

We can see that there are some 'un-agree-able' coefficients between the models. This mean that between different artists, the coefficients are affecting the relationship to popularity differently.
Particular, we can see that the explicit interactions are high ranked in the GLM setting in comparison to the GLMM setting. The loudness and the mode seems more important in the GLMM setting. The other mismatches are less significant.
...
## Summary
In this analysis of the Spotify data-set, the goal was to model the relationship between various features and the popularity of songs using different Generalized Linear Models (GLM's). The chosen response variable, popularity, was analysed as a count data and discrete. 
For count data (regression problem) The Poisson model was an initial suitable choice due to its simplicity and compatibility with count data. However, issues of over-dispersion arose, suggesting that the Poisson model might not be the best fit.

The Likelihood Ratio Test (LRT) indicated that a more complex model was superior, yet its complexity led to the exploration of a leaner model. Through backward elimination, the tempo feature was removed. 
This process of fitting the three basic models, and then backward elimination some features was done for all the different GLM's that I've fitted.
In terms of complexity, the final selected model is still pretty big, and for further research, i can try to reduce the size of the model with more 'strong' backward elimination (controlling with higher p-value), or with trying to remove high correlated features.

The final model was defined as
> popularity  ~ duration_min + num_of_artists + artist_popularity + valence + danceability + energy + explicit + key + mode + time_signature + loudness + acousticness + instrumentalness + mode:loudness + mode:instrumentalness + energy:explicit + explicit:speechiness + explicit:liveness + explicit:acousticness + valence:explicit + energy:mode + energy:speechiness + mode:time_signature

and its metrics were evaluated: AIC, BIC, and deviance were provided for model comparison. The model's coefficients were analyzed, revealing the significance and strength of various covariates. 
...
Our results also suggest that some factors, such as key and time signature, which not have a significant impact on song popularity. This is interesting, as these factors are often thought to be important in music theory, i suspect that there is not enough data for the variability of this factors.
...
Only in the end of my analsis, I've remembered that i didnt try the regular linear model. Surprisingly, the linear model had much better metrics that the poisson and the negative binomial models. 

The Logistic model was also explored with the same formulas, the backward elimination procedure assisted in constructing a smaller and more interpret-able model. The logit link function was deemed optimal in terms of AIC and BIC.
Although dealing with different problem from count data, i wanted to see the difference between the coeffecients in the negative-binomial model and the logistic model. Note that those model are dealing with different data (in the logistic case we have songs with popularity < 20). I'm assuming that the difference in the coefficients will be caused by the fact that the logistic model will be focused on separability of bad and good songs.
In the logistic model there were a few more interesting coefficients that arose like:
...
num_of_Artist - Negative estimate, probably there are a lot of unpopular songs with a lot of artists in the same song.


A logistic GLMM demonstrated significant improvement over the logistic model (LRT $\chi^2_{stat} = 2841.3$). While coefficient estimates and significance varied, the GLMM's complexity allowed it to account for more factors affecting song popularity, making it the preferred choice. Notably, comparing coefficient rankings revealed discrepancies, highlighting the impact of artist-specific effects on interpreting predictor-popularity relationships.
...
3. Try different data - see if the conclusions here can be generalized to different time period.
...
### Code
Note that most of the EDA and preprocessing code was done in python, I've added a python code in a seperate file.
This paper is fully reproducible, you can open it in rmarkdown and run it (the code is hidden in the pdf, but it exist in the notebook).
todo add code.

```
saldkngfsadlfnasdlfnasdf
aslgknsdl'gndsknds
sagnaslgnlgnas
fdsfds
```
